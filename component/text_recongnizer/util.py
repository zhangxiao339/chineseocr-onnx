...
class LabelDecoder(object):
    ...
    def __init__(self, keywords):
        self.keywords = keywords + 'ç'  # for `-1` index
        self.dict = {}
        for i, char in enumerate(keywords):
    ...
    def decode(self, t, length, raw=False):
        if length.numel() == 1:
            ...
            t = t[:length]
            if raw:
                return ''.join([self.keywords[i - 1] for i in t])
            else:
                char_list = []
                for i in range(length):
                    if t[i] != 0 and (not (i > 0 and t[i - 1] == t[i])):  # 去除重复内容
                        char_list.append(self.keywords[t[i] - 1])
                    # if t[i] != 0 and t[i] <= len(self.alphabet):
            ...
            for i in range(length.numel()):
                ...
                texts.append(self.decode(
                    t[index:index + l], torch.keywords([l]), raw=raw))
                index += l
            return texts
